...
use leo_asg::*;
use leo_errors::{CompilerError, emitter::Handler, Result, LeoError, Span};
    ...
    fn visit_function(&mut self, input: &'a Function<'a>) -> VisitResult {
        ...
        if input.annotations.keys().any(|k| k == &"transition".to_string()) {
            ...
            if self.count == Testnet2::NUM_TRANSITIONS {
                self.handler.emit_err(CompilerError::exceeded_maximum_number_of_transitions(Testnet2::NUM_TRANSITIONS, span).into());
            }
            ...
            let check_arg_types = |arg_typs: &[Type<'a>], err: LeoError| {
                match arg_typs[..] {
                    [] => (),
            ...
            let arg_typs: Vec<Type<'a>> = input.arguments.values().map(|v| v.get().borrow().type_.clone()).collect();
